...
export class PedidoService {
  ...
  private trataDadosDoPedido(
    ...
    dadosDoPedido.itensPedido.forEach((itemPedido) => {
      ...
      if (itemPedido.quantidade > produtoRelacionado.quantidadeDisponivel) {
        throw new BadRequestException(`A quantidade solicitada (${itemPedido.quantidade}) é maior do que a disponivel 
          (${produtoRelacionado.quantidadeDisponivel}) para o produto ${produtoRelacionado.nome}`);
      }
    });
  }
  ...
  async buscaPedidoUsuario(usuarioId: string) {
  ...
  async atualizaPedido(id: string, dadosDoPedido: AtualizaPedidoDTO) {
    const pedido = await this.pedidoRepository.findOneBy({ id });

    if (!pedido) {
      throw new NotFoundException('Pedido não encontrado ou já foi concluído');
    }
